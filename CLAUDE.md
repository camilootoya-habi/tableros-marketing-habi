...
## ⚠ Un solo clon de este repo (25-sep, entrega del proyecto de Nicolás)

- La carpeta de trabajo es **`~/habi/tableros-marketing-habi`**, con remoto
  `camilootoya-habi/tableros-marketing-habi`. Trabaja siempre desde una rama nueva sobre
  `origin/main` y abre el PR contra ESTE repo.
- Las carpetas `~/habi/tableros-marketing-habi (desde carpeta habi)` y
  `~/habi/tablero-neon-optim` se **borraron a propósito**: eran clones del fork viejo
  `notero-88/tableros-marketing-habi` (rama `pool-ab-view`, ~2.349 commits detrás de `main`).
  No las recrees. Un cambio hecho ahí terminaba en el fork y no en el tablero publicado.
- El fork `notero-88/tableros-marketing-habi` está **obsoleto**: nadie publica ahí (el ciclo
  diario dejó de hacerlo el 25-sep). No abras PRs contra él.

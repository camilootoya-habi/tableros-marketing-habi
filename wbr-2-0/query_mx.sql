-- WBR 2.0 — Métricas diarias por fuente (MX)
...
WITH
  ...
  -- completos (lead_forms): estado actual ≠ incompleto, por fecha de registro (ver query.sql CO).
  completos_agg AS (
  ...
  -- Estudio Inmueble: paso por 65 por fecha de ENTRADA a 65 (evento); de esos, los calificados.
  ev_65 AS (
    SELECT deal_id AS negocio_id, MIN(date_create) AS ts
    FROM `sellers-main-prod.mx_rds_staging.habi_db_history_state`
    WHERE state_id = 65
    GROUP BY 1
  ),

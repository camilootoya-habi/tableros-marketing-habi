...
def cosecha_serie(sendlog, mart_by_msgid, inbound_phones, interesado_phones, interesado_nocreado_phones, tipo, n=20):
    """Cosecha por período de ENVÍO (attempted_at, granularidad `tipo`): avance enviados→entregados→leidos→
    respondidos→interesados→interesados_no_creados. Devuelve los `n` períodos más recientes (ascendente).
    interesados_no_creados = interesados cuyo nid aún NO se recreó (no está en la tabla recreation)."""
    from collections import defaultdict
    A=defaultdict(lambda: dict(enviados=0,entregados=0,leidos=0,respondidos=0,interesados=0,interesados_no_creados=0))
    for r in sendlog:
        ...
        if r.get("phone") in interesado_nocreado_phones: a["interesados_no_creados"]+=1

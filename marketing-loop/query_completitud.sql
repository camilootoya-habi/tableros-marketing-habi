...
SELECT 'MX' AS pais, re.fecha AS fecha_creacion,
  ...
  IF(COALESCE(p.half_bathroom,0)>0,1,0) AS c_medios_banos,
  IF(COALESCE(p.room_num,0)>0,1,0) AS c_habitaciones,
  IF(COALESCE(p.garage,0)>0,1,0) AS c_garaje,
  IF(COALESCE(p.elevator,0)>0,1,0) AS c_ascensor,
...
SELECT 'CO' AS pais, re.fecha AS fecha_creacion,
  ...
  IF(COALESCE(tiv.garajes,0)>0,1,0) AS c_garaje,
  IF(COALESCE(tiv.num_ascensores,0)>0,1,0) AS c_ascensor,
